  ...
  return (
    <div className="border border-trait rounded-md bg-surface overflow-hidden">
      <div className="px-4 py-3 border-b border-trait">
        <p className="text-encre text-sm font-medium">{title}</p>
      </div>
...
export default function SimParametresAvances({ values, advanced, calculs, formatCurrency }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border border-trait rounded-md bg-surface overflow-hidden">
        <div className="px-4 py-3 border-b border-trait">
          <p className="text-encre text-sm font-medium">Impact sur les indicateurs</p>
        </div>
        <div className="flex flex-wrap divide-x divide-trait">
      ...
      <Card title="Vacance locative">
        ...
        <div className="flex flex-wrap gap-1.5 mt-3">
          {advanced.vacancesLocatives.map((v, i) => v > 0 && (
            <span key={i} onClick={() => { const n = [...advanced.vacancesLocatives]; n[i] = 0; advanced.setVacancesLocatives(n); }} className="text-[11px] bg-menthe/15 text-menthe px-2 py-1 rounded cursor-pointer">An {i + 1}: {v}m ✕</span>
          ))}
          {nbVacance === 0 && <span className="text-[11px] text-brume">Aucune vacance renseignée</span>}
        </div>
      </Card>
      ...
      <Card title="Travaux bailleur">
        ...
        <div className="flex flex-wrap gap-1.5 mt-3">
          {advanced.travauxBailleur.map((t, i) => t > 0 && (
            <span key={i} onClick={() => { const n = [...advanced.travauxBailleur]; n[i] = 0; advanced.setTravauxBailleur(n); }} className="text-[11px] bg-menthe/15 text-menthe px-2 py-1 rounded cursor-pointer">An {i + 1}: {t.toLocaleString("fr-FR")} € ✕</span>
          ))}
          {totalTravaux === 0 && <span className="text-[11px] text-brume">Aucun travaux renseigné</span>}
        </div>
      </Card>
    </div>
  );
