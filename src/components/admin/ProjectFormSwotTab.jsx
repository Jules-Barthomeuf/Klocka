      ...
      <div className="p-5 bg-white/[0.015] rounded-md border border-[#131c1b] space-y-3">
        ...
        {liens.map((l, i) => (
          <div key={i} className="flex gap-2">
            <Input value={l.label || ""} onChange={(e) => { const n = [...liens]; n[i] = { ...n[i], label: e.target.value }; updateLiens(n); }} placeholder="Label" className="bg-[#161616] text-white border-[#1c2725] w-1/3" />
            <Input value={l.url || ""} onChange={(e) => { const n = [...liens]; n[i] = { ...n[i], url: e.target.value }; updateLiens(n); }} placeholder="URL" className="bg-[#161616] text-white border-[#1c2725] flex-1" />
        ...
        <Button variant="outline" size="sm" onClick={() => updateLiens([...liens, { label: "", url: "" }])} className="border-[#16201f] text-white/30 hover:text-white">
