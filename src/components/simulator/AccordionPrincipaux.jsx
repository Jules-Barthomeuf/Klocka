...
export default function AccordionPrincipaux({ 
  ...
  return (
    <div className="mb-4">
      <Card className="bg-surface border border-encre/[0.1] rounded-md overflow-hidden">
        <CardHeader className="p-5 pb-3">
          <CardTitle className="font-light text-encre text-lg md:text-xl tracking-tight">Paramètres principaux</CardTitle>
        </CardHeader>
        <CardContent className="p-5 pt-0 space-y-4">
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label className="text-encre text-sm flex items-center">Surface <TooltipInfo field="surface" /></Label>
            ...
            <input
              ...
              onChange={(e) => setSurface(Number(e.target.value))}
              disabled={selectedProjectId && selectedProjectId !== "default"}
              className="w-full h-1 bg-bord rounded-full appearance-none cursor-pointer accent-menthe disabled:opacity-50 disabled:cursor-not-allowed"
            />
          </div>
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label className="text-encre text-sm flex items-center">Loyer initial HC HT <TooltipInfo field="loyerInitialHTHC" /></Label>
              <div className="flex items-center gap-1">
                {user?.role === "admin" && !selectedProjectId ? (
                  <CalculatorInput
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label className="text-encre text-sm flex items-center">Prix du bien FAI <TooltipInfo field="prixBienFAI" /></Label>
              <div className="flex items-center gap-1">
                {user?.role === "admin" && !selectedProjectId ? (
                  <CalculatorInput
                    value={prixBienFAI}
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label className="text-encre text-sm flex items-center">Prix négocié FAI <TooltipInfo field="prixBienNegocie" /></Label>
              <div className="flex items-center gap-1">
                {user?.role === "admin" ? (
                  <CalculatorInput
            ...
            <input type="range" min="50000" max="2000000" step="10000" value={prixBienNegocie} onChange={(e) => setPrixBienNegocie(Number(e.target.value))} className="w-full h-1 bg-bord rounded-full appearance-none cursor-pointer accent-menthe" />
          </div>
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label className="text-encre text-sm flex items-center">Durée crédit <TooltipInfo field="dureeCredit" /></Label>
              <div className="flex items-center gap-1">
                {user?.role === "admin" ? (
                  <CalculatorInput
                    value={dureeCredit}
            ...
            <input type="range" min="5" max="30" step="1" value={dureeCredit} onChange={(e) => setDureeCredit(Number(e.target.value))} className="w-full h-1 bg-bord rounded-full appearance-none cursor-pointer accent-menthe" />
          </div>
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label className="text-encre text-sm flex items-center">Apport <TooltipInfo field="apport" /></Label>
              <div className="flex items-center gap-1">
                {user?.role === "admin" ? (
                  <CalculatorInput
                    value={apport}
            ...
            <input type="range" min="0" max="500000" step="5000" value={apport} onChange={(e) => setApport(Number(e.target.value))} className="w-full h-1 bg-bord rounded-full appearance-none cursor-pointer accent-menthe" />
          </div>
        </CardContent>
      </Card>
    </div>
