  ...
  return (
    <>
      ...
      <Section id="presentation" titre="1 · Présentation investisseur" droite="Même fiche de deal, présentée selon le profil ciblé">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
          <div>
            ...
            <ul className="m-0 mt-2 p-0 list-none">
              {p.pitch.slice(1).map((l, i) => <li key={i} className="py-3 border-b border-relief text-[15px] text-encre flex items-start gap-3"><span className="text-brume text-[11px] mt-[7px]">▸</span>{l}</li>)}
            </ul>
            <p className="m-0 mt-4 text-[12.5px] text-ardoise">Ce qui compte : {p.accent} Les risques sont présentés avec leur chiffrage, sans les minorer.</p>
          </div>
          <div>
            <Mono>Format · <span className="normal-case tracking-normal text-craie">{{ fiche: "Fiche de deal mise en page", mail: "Message court", pitch: "Support de call" }[format]}</span></Mono>
            <div className="mt-2 space-y-2">
              {[["fiche", "PDF", "Fiche de deal mise en page", "Une à deux pages prêtes à joindre à un mail : résumé du bien, risques retenus, prix recommandé, conditions, pour et contre."], ["mail", "Mail", "Message court", "Les points clés dans le corps du mail, la fiche en pièce jointe. Personnalisé avec les critères de l'investisseur."], ["pitch", "Présentation", "Support de call", "Support pour un call ou une réunion : chiffres clés, risques, prix, pour et contre."]].map(([id, tag, t, d]) => (
                <button key={id} onClick={() => setFormat(id)} className={`w-full text-left px-4 py-3 border transition-colors ${format === id ? "border-menthe/60" : "border-trait hover:border-bord-doux"}`}>
                  <p className="m-0 text-[13.5px] font-semibold text-encre"><Mono className="mr-2">{tag}</Mono>{t}</p>
                  <p className="m-0 mt-1 text-[12.5px] leading-[1.5] text-ardoise">{d}</p>
                </button>
              ))}
            </div>
        ...
        {edition && (
          <div className="mt-5 border border-trait">
            <div className="flex items-center justify-between px-4 py-2 border-b border-trait"><Mono>{{ fiche: "Fiche de deal", mail: "Mail", pitch: "Support de call" }[format]} · {p.titre}</Mono><button onClick={() => copier(brouillon, "Le texte")} className="inline-flex items-center gap-1.5 text-[12.5px] text-craie hover:text-encre"><Copy className="w-3.5 h-3.5" /> Copier</button></div>
            <textarea value={brouillon} onChange={(x) => setBrouillon(x.target.value)} rows={Math.min(28, Math.max(8, brouillon.split("\n").length + 1))} className="w-full bg-transparent border-0 outline-none resize-y px-4 py-3 text-[13.5px] leading-[1.7] text-encre font-[inherit]" />
          </div>
        )}
      </Section>

      {/* La timeline */}
      <div className="px-6 max-md:px-4 py-5 border-b border-trait">
        ...
        {e.timeline.length ? (
          <ul className="m-0 p-0 list-none relative pl-5">
            <span className="absolute left-[5px] top-1 bottom-1 w-px bg-bord-doux" />
            {e.timeline.map((t, i) => { const [c, l] = ACTEUR[t.acteur] || ACTEUR.systeme; return (
              <li key={i} className="relative py-1.5 flex items-baseline gap-3 text-[12.5px]">
                <span className="absolute -left-5 top-[11px] w-[11px] h-[11px] rounded-full border-2 border-fond" style={{ background: c }} />
                <span className="text-ardoise tabular-nums w-[44px] flex-none">{quand(t.date)}</span>
                <span className="text-encre min-w-0">{t.libelle}</span>
                <span className="text-[11px] tracking-[.12em] uppercase flex-none" style={{ color: c }}>{l}</span>
              </li>
            ); })}
          </ul>
        ) : <p className="m-0 text-[12.5px] text-ardoise">Rien encore.</p>}
      </div>
      ...
      <div className="px-6 max-md:px-4 py-5 border-b border-trait">
        ...
        {!e.complements.depuis ? <p className="m-0 text-[12.5px] text-ardoise">La fiche sera photographiée au passage à l'étape 3 ; les compléments se mesurent contre elle.</p>
          : !e.complements.lignes.length ? <p className="m-0 text-[12.5px] text-ardoise">{e.complements.pieces.length ? `${e.complements.pieces.join(", ")} — sans changement sur la fiche.` : "Aucun complément depuis l'étape 3."}</p>
          : (
            <table className="w-full border-collapse text-[12.5px]">
              <thead><tr className="text-[11px] tracking-[.14em] uppercase text-brume"><th className="text-left font-normal py-1.5 pr-3">Champ</th><th className="text-left font-normal py-1.5 px-3">Ancienne valeur</th><th className="text-left font-normal py-1.5 px-3">Nouvelle valeur</th><th className="text-left font-normal py-1.5 pl-3">Source</th></tr></thead>
              <tbody>{e.complements.lignes.map((l) => (
                <tr key={l.champ} className="border-t border-relief">
                  ...
                  <td className="py-2 pl-3 text-[12.5px] text-brume">{l.source || ""}</td>
                </tr>
              ))}</tbody>
            </table>
          )}
      ...
      <div className="px-6 max-md:px-4 py-5">
        ...
        {e.conclusion ? (
          <p className="m-0 text-[13.5px] text-encre">{{ signe: "Signé", perdu: "Perdu", abandonne: "Abandonné" }[e.conclusion.etat]}{e.conclusion.motif ? ` — ${e.conclusion.motif}` : ""} <span className="text-brume text-[12.5px]">· {new Date(e.conclusion.le).toLocaleString("fr-FR", { dateStyle: "short", timeStyle: "short" })}{e.conclusion.par ? ` · ${e.conclusion.par}` : ""}</span></p>
        ) : (
          <>
            ...
            <div className="grid sm:grid-cols-3 gap-4 max-w-[860px]">
              {[["signe", "Signé", "Le dossier part dans les dossiers signés ; le projet vit sur la plateforme.", "border-menthe/50"], ["perdu", "Perdu", "Archives ; le prix et le loyer alimentent la base marché.", "border-ambre/50"], ["abandonne", "Abandonné", "Archives ; le motif alimente la base marché.", "border-alerte/50"]].map(([etat, l, d, b]) => (
                <button key={etat} onClick={() => !apercu && conclure.mutate(etat)} disabled={apercu || conclure.isPending} className={`text-left rounded-md p-4 border ${b} hover:bg-encre/[0.03] disabled:opacity-50`}>
                  <p className="m-0 text-[13.5px] font-semibold text-encre">{l}</p>
                  <p className="m-0 mt-1 text-[12.5px] leading-[1.5] text-ardoise">{d}</p>
                </button>
              ))}
            </div>
