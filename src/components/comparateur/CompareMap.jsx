...
export default function CompareMap({ metrics }) {
  ...
  if (projectsWithCoords.length === 0 && !loading) {
    return (
      <div className="bg-[#000000] rounded-md border border-[#1f2228] p-5">
        ...
        <div className="flex flex-col items-center justify-center h-48 text-[#f2f3f5]/20">
  ...
  if (loading && projectsWithCoords.length === 0) {
    return (
      <div className="bg-[#000000] rounded-md border border-[#1f2228] p-5">
        <div className="flex items-center mb-4">
          <p className="text-[#f2f3f5]/40 text-xs uppercase tracking-[0.15em]">Localisation des projets</p>
        </div>
        <div className="flex items-center justify-center h-48 text-[#f2f3f5]/30">
            ...
            return (
              <Marker key={m.id} position={[m.lat, m.lng]} icon={icon}>
                <Popup>
                  ...
                  <div className="text-xs text-[#9298a6]">{m.adresse || ""}</div>
                </Popup>
              </Marker>
            );
          })}
