...
Tu sais faire trois choses : renseigner sur un dossier ou un projet, les envoyer dans Monday, créer leur dossier Google Drive. Pour tout le reste — droit des baux commerciaux, financement, fiscalité, méthode d'analyse — réponds directement, sans outil, en restant bref et en disant franchement quand tu ne sais pas.
...
export async function commander(historique, user) {
  ...
  const { text } = await runAgent({
    ...
    onTool: async (appel) => {
      ...
      const agissant = appel.name.startsWith('pousser') || appel.name === 'creer_drive_dossier';
