...
export default function ShadowEditorDialog({ open, onOpenChange, project, shadowRecord, users, initialTab, initialViewMode }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto bg-[#000000] border-[#1f2228]" onInteractOutside={(e) => e.preventDefault()}>
        <DialogHeader>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-purple-400 uppercase tracking-[0.2em] text-[10px] font-medium mb-1">Shadow Mode</p>
              <DialogTitle className="text-2xl font-light text-[#f2f3f5] tracking-tight">
                {project?.titre}
              </DialogTitle>
            </div>
            <Button variant="ghost" size="icon" onClick={() => onOpenChange(false)} className="text-[#9298a6] hover:text-[#f2f3f5] hover:bg-[#f2f3f5]/5">
              <X className="w-5 h-5" />
            </Button>
          </div>
        </DialogHeader>
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-md border text-sm transition-all ${
              viewMode === "current"
                ...
                : "bg-[#000000] border-[#1f2228] text-[#9298a6] hover:text-[#f2f3f5]"
            }`}
          >
        ...
        <div>
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="grid w-full grid-cols-10 bg-[#000000] border border-[#1f2228] rounded-md p-1">
              <TabsTrigger value="informations" className="data-[state=active]:bg-[#f2f3f5]/10 data-[state=active]:text-[#f2f3f5] text-[11px] rounded-lg">Infos</TabsTrigger>
              <TabsTrigger value="environnement" className="data-[state=active]:bg-[#f2f3f5]/10 data-[state=active]:text-[#f2f3f5] text-[11px] rounded-lg">Env.</TabsTrigger>
              <TabsTrigger value="secteur" className="data-[state=active]:bg-[#f2f3f5]/10 data-[state=active]:text-[#f2f3f5] text-[11px] rounded-lg">Secteur</TabsTrigger>
              <TabsTrigger value="locataire" className="data-[state=active]:bg-[#f2f3f5]/10 data-[state=active]:text-[#f2f3f5] text-[11px] rounded-lg">Locataire</TabsTrigger>
              <TabsTrigger value="copropriete" className="data-[state=active]:bg-[#f2f3f5]/10 data-[state=active]:text-[#f2f3f5] text-[11px] rounded-lg">Copro</TabsTrigger>
              <TabsTrigger value="marche" className="data-[state=active]:bg-[#f2f3f5]/10 data-[state=active]:text-[#f2f3f5] text-[11px] rounded-lg">Marché</TabsTrigger>
              <TabsTrigger value="diagnostique" className="data-[state=active]:bg-[#f2f3f5]/10 data-[state=active]:text-[#f2f3f5] text-[11px] rounded-lg">Diag</TabsTrigger>
              <TabsTrigger value="docs_projet" className="data-[state=active]:bg-[#f2f3f5]/10 data-[state=active]:text-[#f2f3f5] text-[11px] rounded-lg">Docs</TabsTrigger>
              <TabsTrigger value="images" className="data-[state=active]:bg-[#f2f3f5]/10 data-[state=active]:text-[#f2f3f5] text-[11px] rounded-lg">Images</TabsTrigger>
              <TabsTrigger value="simulateur" className="data-[state=active]:bg-[#f2f3f5]/10 data-[state=active]:text-[#f2f3f5] text-[11px] rounded-lg">Simu</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="secteur" className="space-y-6 mt-6">
              <div className="space-y-4">
                ...
                <p className="text-[#9298a6] text-sm">Utilisez les onglets Infos / Locataire / Copro pour les champs principaux.</p>
              </div>
            </TabsContent>
        ...
        {/* Save button */}
        <div className="flex justify-end gap-3 mt-6 pt-6 border-t border-[#1f2228]">
          <button onClick={() => onOpenChange(false)} className="px-5 py-2.5 text-[#9298a6] hover:text-[#f2f3f5] text-sm transition-colors">Annuler</button>
          {viewMode === "shadow" ? (
            <button
              onClick={handleSaveShadow}
              disabled={saveShadowMutation.isPending}
              className="flex items-center gap-2 bg-purple-500/15 border border-purple-500/30 hover:bg-purple-500/25 text-[#f2f3f5] px-6 py-2.5 rounded-md transition-all text-sm disabled:opacity-40"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 bg-blue-500/15 border border-blue-500/30 hover:bg-blue-500/25 text-[#f2f3f5] px-6 py-2.5 rounded-md transition-all text-sm disabled:opacity-40"
            >
