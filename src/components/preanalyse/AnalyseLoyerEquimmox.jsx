...
export default function AnalyseLoyerEquimmox({ dossier, lot, apercu = false, onRefresh }) {
  ...
  const chercher = useMutation({
    ...
    onSuccess: (r) => {
      ...
      if (r.resultat?.du_cache) toast.message("Analyse déjà connue", { description: "Résultat gardé de moins de trente jours. « Relire » interroge Equimmox de nouveau." });
      onRefresh?.();
    },
    onError: (e) => toast.error(e?.message || "Equimmox n'a pas répondu"),
  });
          ...
          <div className="grid grid-cols-3 gap-3">
            {[["Bas", resultat.bas, "bas"], ["Moyenne", resultat.moyenne, "moyenne"], ["Haut", resultat.haut, "haut"]].map(([l, n, cle]) => (
              <div key={cle} className="border border-[#1f2228] rounded-xl px-4 py-3 bg-[#0f1114]">
                ...
                <p className={`m-0 mt-1 text-[20px] tabular-nums font-light ${dedans(cle) ? "text-[#d9b46a]" : "text-[#f2f3f5]"}`}>{euros(n)}</p>
              </div>
            ))}
          </div>
