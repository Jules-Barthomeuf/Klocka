...
export default function BoiteSaisie({
  ...
  return (
    <div
      {...propsConteneur}
      className={`relative rounded-[20px] shadow-[0_18px_50px_rgba(0,0,0,.45)] ${lumiere ? "p-px overflow-hidden bg-encre/[0.12] [.glisse_&]:bg-menthe" : ""} ${classeConteneur}`}
    >
      {lumiere && (
        <div
          aria-hidden="true"
    ...
    >
      ...
      <div className={`flex items-center justify-between gap-4 border-t border-trait ${compact ? "px-4 py-2" : "px-7 max-md:px-5 py-4"}`}>
        <div className="flex flex-wrap items-center gap-2 min-w-0 text-ardoise">{gauche}</div>
