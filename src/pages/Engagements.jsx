...
const TEINTE = (e) =>
  e.en_retard
    ? { filet: "border-[#c4715c]", etiquette: "text-[#c4715c]" }
    : e.echeance
      ? { filet: "border-[#d9c08a]", etiquette: "text-[#d9c08a]" }
      : { filet: "border-[#4a4844]", etiquette: "text-[#8b8880]" };
      ...
      <div className="max-w-[1100px] mx-auto px-5 md:px-12 py-10 md:py-16">
        ...
        <header className="flex flex-wrap items-start justify-between gap-x-10 gap-y-6">
          ...
          <div className="flex items-start gap-10 max-md:gap-7">
            ...
            <div className="text-right">
              <p className={`m-0 text-[38px] max-md:text-[30px] font-light leading-none ${enRetard.length ? "text-[#c4715c]" : "text-[#4a4844]"}`}>
                {enRetard.length}
              </p>
              <p className="m-0 mt-2.5 text-[10.5px] tracking-[.16em] uppercase text-[#8b8880]">
                En retard
            ...
            {ouverts.map((e) => {
              ...
              return (
                <div key={e.id} className={`border-l-2 ${t.filet} pl-6`}>
                  ...
                  <p className="m-0 mt-2 text-[19px] max-md:text-[17px] font-light leading-[1.45] text-[#e8e4dd]">
                    {e.quoi}
                  </p>
                  <p className="m-0 mt-1.5 text-[13px] text-[#8b8880]">
                    ...
                    {reportEn === e.id ? (
                      <span className="inline-flex items-center gap-2">
                        <input
                          ...
                          className="bg-transparent border border-[#2f2c29] px-2.5 py-1.5 text-[12.5px] text-[#e8e4dd] outline-none [color-scheme:dark]"
                        />
                        <button
                          onClick={() => { if (nouvelleDate) { setOccupe(e.id); repousser.mutate({ id: e.id, date: nouvelleDate }); } }}
                          disabled={!nouvelleDate || !!occupe}
                          className="px-3 py-2 bg-[#d9c08a] text-[#0b0a09] text-[10.5px] tracking-[.16em] uppercase disabled:opacity-40"
                        >
                          Valider
                        </button>
                        <button onClick={() => setReportEn(null)} className="text-[12px] text-[#6f6c66] hover:text-[#b9b5ad]">
                          annuler
                        </button>
                      </span>
                    ) : (
                      <button
                        ...
                        className="px-3.5 py-2 border border-[#2f2c29] text-[10.5px] tracking-[.16em] uppercase text-[#b9b5ad] hover:border-[#54504a] transition-colors"
                      >
                        Repousser
                      </button>
                    )}
        ...
        {tenus.length > 0 && (
          <>
            <div className="h-px bg-[#232120] my-12 max-md:my-9" />
            <p className="m-0 mb-7 text-[11px] tracking-[.18em] uppercase text-[#8b8880]">Tenus récemment</p>
            <div className="space-y-4">
              {tenus.map((e) => (
                <div key={e.id} className="flex flex-wrap items-baseline gap-x-4 gap-y-0.5">
                  <span className="text-[14px] text-[#6f6c66] line-through decoration-[#3a3835]">{e.quoi}</span>
