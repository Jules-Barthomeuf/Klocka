...
export default function IndicateursPerformance({ calculs, anneeRevente, formatCurrency, textClass, mutedClass }) {
  return (
    <div className="bg-[#0a0c0c] p-4 rounded-md md:p-6 border border-[#edeae5]/[0.1] max-w-full overflow-hidden">
      <h3 className="font-light text-[#edeae5] text-xl md:text-2xl tracking-tight mb-3 md:mb-4">Performance globale</h3>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4 w-full">
        ...
        <div className="p-2 md:p-4 rounded-md bg-[#edeae5]/[0.04] border border-[#282b2a] flex flex-col items-center justify-center text-center">
          ...
          <p className="text-xs text-[#edeae5]/50 mt-1">soit {formatCurrency(Math.round(calculs.indicateurs.creationRichesseBrute / anneeRevente / 12))}/mois</p>
        </div>
      </div>
    </div>
  );
