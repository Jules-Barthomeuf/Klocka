  ...
  return (
    <section>
      <div className="flex flex-wrap items-center justify-between gap-2 mb-2.5">
        <h3 className="m-0 text-[15px] font-medium text-encre">Simulateur</h3>
        {lots.length > 1 && (
          <select
            value={index}
            onChange={(e) => setIndex(Number(e.target.value))}
            className="bg-surface border border-trait rounded-md px-2.5 py-1.5 text-[12.5px] text-ardoise outline-none hover:border-bord-vif transition-colors max-w-[240px]"
          >
            {lots.map((l, i) => (
      ...
      {!lot && (
        <p className="m-0 mb-2.5 text-[11px] text-brume leading-[1.5]">
