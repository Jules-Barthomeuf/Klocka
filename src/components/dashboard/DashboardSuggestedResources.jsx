...
export default function DashboardSuggestedResources({ user }) {
  ...
  return (
    ...
    >
      ...
      <div
        onClick={handleOpen}
        className="group cursor-pointer flex gap-4 bg-fond border border-encre/[0.12] hover:border-menthe/60 transition-colors duration-300 overflow-hidden"
      >
        {/* Thumbnail */}
        <div className="relative w-40 md:w-52 flex-shrink-0 overflow-hidden bg-encre/[0.02]">
          {resource.image_miniature ? (
            ...
            <div className="w-full h-full min-h-[100px] flex items-center justify-center">
              <Play className="w-10 h-10 text-menthe/30" />
            </div>
          )}
          <div className="absolute inset-0 flex items-center justify-center bg-fond/30 group-hover:bg-fond/20 transition-colors">
            <div className="w-10 h-10 rounded-full bg-encre/20 backdrop-blur-sm flex items-center justify-center group-hover:scale-110 transition-transform">
              <Play className="w-4 h-4 text-encre fill-white ml-0.5" />
            </div>
          </div>
          {viewedResources.includes(resource.id) && (
            <div className="absolute top-2 right-2 w-5 h-5 rounded-full bg-menthe flex items-center justify-center">
              <CheckCircle2 className="w-3 h-3 text-encre" />
            </div>
          )}
