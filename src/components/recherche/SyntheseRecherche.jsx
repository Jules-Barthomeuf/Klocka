...
function BudgetColumn({ profil }) {
  ...
  return (
    <div className="bg-[#0e100f] border border-[#edeae5]/[0.12] overflow-hidden">
      {/* Header */}
      <div className="px-5 py-4 border-b border-[#242726]" style={{ backgroundColor: `${color}10` }}>
        <span
        ...
        <p className="text-[#edeae5] text-sm font-medium leading-snug">{titre}</p>
      </div>
      ...
      <div className="px-5 py-4 space-y-5">
        ...
        {sections.map((section, sIdx) => (
          <div key={sIdx}>
            <div className="flex items-center gap-2 mb-2.5">
              ...
              <p className="text-[9px] uppercase tracking-[0.15em] text-[#edeae5]/40 font-medium">{section.titre}</p>
            </div>
            <div className="space-y-2">
              {section.items.map((item, idx) => (
                <div key={idx} className="pl-3 border-l-2 border-[#242726]">
                  <p className="text-xs font-medium text-[#edeae5]/80">{item.label}</p>
                  <p className="text-[11px] text-[#edeae5]/35 leading-relaxed whitespace-pre-line">{item.desc}</p>
                </div>
              ))}
            </div>
        ...
        <div>
          <div className="flex items-center gap-2 mb-2.5">
            ...
            <p className="text-[9px] uppercase tracking-[0.15em] text-[#edeae5]/40 font-medium">Rendements cibles</p>
          </div>
          <div className="space-y-2">
            {rendements.map((r, idx) => (
              <div key={idx} className="flex items-center justify-between">
                <span className="text-xs text-[#edeae5]/50">{r.label}</span>
        ...
        {typesCommerceExclus && typesCommerceExclus.length > 0 && (
          <div>
            <div className="flex items-center gap-2 mb-2.5">
              ...
              <p className="text-[9px] uppercase tracking-[0.15em] text-[#edeae5]/40 font-medium">Exclusions</p>
            </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#0e100f] border border-[#edeae5]/[0.12] p-5">
        <div className="flex items-center gap-3 mb-2">
          <div className="w-9 h-9 rounded-md bg-[#35a79b]/10 flex items-center justify-center">
            <Building2 className="w-5 h-5 text-[#35a79b]" />
          </div>
          <div>
            <h3 className="text-[#edeae5] text-sm font-medium">Vue synthétique</h3>
            <p className="text-[#edeae5]/30 text-xs">Critères de recherche par tranche de budget — toutes catégories</p>
          </div>
        </div>
      </div>
