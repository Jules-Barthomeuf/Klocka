...
function Carte({ ressource, onOpen }) {
  ...
  return (
    <button
      ...
      className="group text-left w-full border-t border-[#f2f3f5]/[0.12] pt-4 pb-4 transition-colors hover:border-[#96c0b8]/60"
    >
      <div className="flex items-baseline justify-between gap-4">
        <span className="text-[15px] max-md:text-[14px] text-[#f2f3f5] group-hover:text-[#c3ddd6] transition-colors">{ressource.titre}</span>
  ...
  if (chargement) {
    return (
      <div className="h-full flex flex-col items-center justify-center gap-3 text-[#9298a6]">
        <Loader2 className="w-5 h-5 animate-spin text-[#96c0b8]" />
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center gap-4 text-center px-8">
        ...
        <a href={url} target="_blank" rel="noopener noreferrer"
          className="inline-flex items-center gap-2 text-[11px] tracking-[0.18em] uppercase text-[#c3ddd6] hover:text-[#f2f3f5] transition-colors">
...
export default function AllerPlusLoin({ section, project }) {
  ...
  return (
    <div className="mt-12 max-md:mt-8">
      <div className="text-[10px] tracking-[0.2em] uppercase text-[#c3ddd6] mb-4">Aller plus loin ?</div>
            ...
            <div className="h-full grid md:grid-cols-[minmax(0,380px)_minmax(0,1fr)] max-md:grid-rows-[auto_1fr] min-h-0">
              ...
              <div className="overflow-y-auto px-7 py-6 max-md:px-5 max-md:py-5 md:border-r border-[#f2f3f5]/[0.13]">
                ...
                {ouverte.points.length > 0 && (
                  <>
                    <div className="text-[10px] tracking-[0.2em] uppercase text-[#c3ddd6] mb-3">Ce qui compte pour ce dossier</div>
                    <ul className="list-none pl-0 space-y-3 mb-6">
                      {ouverte.points.map((p, i) => (
                        <li key={i} className="flex gap-2.5 text-[13.5px] leading-[1.7] text-[#c9cdd6]">
                          <span className="w-1 h-1 rounded-full bg-[#96c0b8] flex-shrink-0 mt-[9px]" />
                          <span>{p}</span>
                        </li>
                      ))}
                    </ul>
