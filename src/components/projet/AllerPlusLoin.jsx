...
function Carte({ ressource, onOpen }) {
  ...
  return (
    <button
      ...
      className="group text-left w-full border-t border-[#edeae5]/[0.12] pt-4 pb-4 transition-colors hover:border-[#35a79b]/60"
    >
      <div className="flex items-baseline justify-between gap-4">
        <span className="text-[15px] max-md:text-[14px] text-[#edeae5] group-hover:text-[#7fd3c9] transition-colors">{ressource.titre}</span>
        <span className="text-[10px] tracking-[0.14em] uppercase text-[#6b7270] whitespace-nowrap">{ressource.duree}</span>
      </div>
      <div className="text-[11px] tracking-[0.1em] uppercase text-[#8b9391] mt-1.5">{ressource.source}</div>
      <div className={`grid transition-all duration-300 ${survol ? "grid-rows-[1fr] opacity-100 mt-3" : "grid-rows-[0fr] opacity-0"}`}>
        <p className="overflow-hidden text-[13px] leading-[1.7] text-[#d3d8d6] mb-0">{ressource.resume}</p>
      </div>
    </button>
  );
}
...
function Article({ url, source }) {
  ...
  if (chargement) {
    return (
      <div className="h-full flex flex-col items-center justify-center gap-3 text-[#8b9391]">
        <Loader2 className="w-5 h-5 animate-spin text-[#35a79b]" />
  ...
  if (erreur) {
    return (
      <div className="h-full flex flex-col items-center justify-center gap-4 text-center px-8">
        <p className="text-[13px] leading-[1.7] text-[#8b9391] mb-0">
          {source} ne permet pas la lecture intégrée ({erreur.toLowerCase()})
        </p>
        <a href={url} target="_blank" rel="noopener noreferrer"
          className="inline-flex items-center gap-2 text-[11px] tracking-[0.18em] uppercase text-[#7fd3c9] hover:text-[#edeae5] transition-colors">
...
export default function AllerPlusLoin({ section, project }) {
  ...
  return (
    <div className="mt-12 max-md:mt-8">
      <div className="text-[10px] tracking-[0.2em] uppercase text-[#7fd3c9] mb-4">Aller plus loin ?</div>
      ...
      <Dialog open={!!ouverte} onOpenChange={(v) => !v && setOuverte(null)}>
        <DialogContent className="max-w-6xl w-[94vw] h-[86vh] bg-[#0e100f] border border-[#edeae5]/[0.13] text-[#edeae5] p-0 [&>button]:hidden overflow-hidden">
          {ouverte && (
            <div className="h-full grid md:grid-cols-[minmax(0,380px)_minmax(0,1fr)] max-md:grid-rows-[auto_1fr] min-h-0">
              ...
              <div className="overflow-y-auto px-7 py-6 max-md:px-5 max-md:py-5 md:border-r border-[#edeae5]/[0.13]">
                <div className="text-[10px] tracking-[0.2em] uppercase text-[#8b9391] mb-2">{ouverte.source} · {ouverte.duree}</div>
                <h3 className="font-cormorant text-[26px] max-md:text-[21px] font-light leading-[1.15] text-[#edeae5] mb-4">{ouverte.titre}</h3>
                <p className="text-[13.5px] leading-[1.75] text-[#d3d8d6] border-t border-[#edeae5]/[0.35] pt-4 mb-6">{ouverte.resume}</p>

                {ouverte.points.length > 0 && (
                  <>
                    <div className="text-[10px] tracking-[0.2em] uppercase text-[#7fd3c9] mb-3">Ce qui compte pour ce dossier</div>
                    <ul className="list-none pl-0 space-y-3 mb-6">
                      {ouverte.points.map((p, i) => (
                        <li key={i} className="flex gap-2.5 text-[13.5px] leading-[1.7] text-[#d3d8d6]">
                          <span className="w-1 h-1 rounded-full bg-[#35a79b] flex-shrink-0 mt-[9px]" />
                          <span>{p}</span>
                        </li>
                      ))}
                    </ul>
                ...
                <a href={ouverte.url} target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 text-[10px] tracking-[0.18em] uppercase text-[#8b9391] hover:text-[#edeae5] transition-colors border-t border-[#edeae5]/[0.12] pt-4 w-full">
                  Voir sur le site d'origine <ExternalLink className="w-3.5 h-3.5" />
                </a>
              </div>

              {/* Colonne droite — la page elle-même */}
              <div className="relative min-h-0 bg-[#0a0c0c]">
                <button onClick={() => setOuverte(null)} aria-label="Fermer"
                  className="absolute top-4 right-4 z-10 w-8 h-8 flex items-center justify-center bg-[#0a0c0c]/80 text-[#8b9391] hover:text-[#edeae5] transition-colors">
                  <X className="w-5 h-5" />
                </button>
