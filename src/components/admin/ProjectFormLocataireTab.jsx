...
export default function ProjectFormLocataireTab({ formData, setFormData }) {
  ...
  return (
    <div className="space-y-6 mt-6">
      <h3 className="text-lg mb-4 text-[#f2f3f5]">Locataire</h3>
      <div className="space-y-4">
        ...
        <VerificationField fieldKey="statut_bail" formData={formData} setFormData={setFormData}>
          <FField label="Statut du bail">
            ...
            >
              <SelectTrigger className="bg-transparent border-none text-[#f2f3f5] p-0 h-auto">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
        ...
        <div className="p-4 bg-[#f2f3f5]/[0.02] rounded-lg space-y-4">
          <div className="flex items-center gap-3">
            ...
            />
            <Label className="text-[#f2f3f5]">Vente récente de fonds de commerce</Label>
          </div>
        ...
        <div className="space-y-4 pt-4 border-t border-[#1f2228]">
          <div className="flex items-center justify-between">
            <div>
              <Label className="text-[#f2f3f5] text-lg">Champs personnalisés du résumé</Label>
              <p className="text-[#9298a6] text-sm mt-1">
          ...
          {(!formData.bail_admin_fields || formData.bail_admin_fields.length === 0) && (
            <p className="text-[#9298a6] text-sm text-center py-2">Aucun champ personnalisé. Cliquez sur le bouton ci-dessus pour en ajouter.</p>
          )}
        </div>
        ...
        <div className="space-y-4 pt-4 border-t border-[#1f2228]">
          <div className="flex items-center justify-between">
            <div>
              <Label className="text-[#f2f3f5] text-lg">Réseaux sociaux & Avis</Label>
              <p className="text-[#9298a6] text-sm mt-1">Ajoutez les liens vers les réseaux sociaux et pages d'avis du locataire</p>
            </div>
            <Button
              type="button"
              variant="outline"
          ...
          {(formData.liens_locataire || []).map((lien, idx) => (
            <div key={idx} className="p-4 bg-[#f2f3f5]/[0.02] rounded-lg space-y-3">
              <div className="flex items-center gap-3">
                ...
                >
                  <SelectTrigger className="bg-[#0f1114] border border-[#1f2228] rounded-none text-[#f2f3f5] w-40 h-[52px] px-[18px]">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
          ...
          {(!formData.liens_locataire || formData.liens_locataire.length === 0) && (
            <p className="text-[#9298a6] text-sm text-center py-2">Aucun lien ajouté.</p>
          )}
        </div>
        ...
        <div className="space-y-4 pt-4 border-t border-[#1f2228]">
          <div className="flex items-center justify-between">
            <div>
              <Label className="text-[#f2f3f5] text-lg">Bilans du locataire</Label>
              <p className="text-[#9298a6] text-sm mt-1">Importez les bilans financiers de l'entreprise (PDF, images...)</p>
            </div>
            <label className="cursor-pointer">
              <input type="file" className="hidden" onChange={handleBilanUpload} accept=".pdf,.png,.jpg,.jpeg,.xlsx,.xls" />
              <Button type="button" variant="outline" size="sm" className="border-[#1f2228] text-[#f2f3f5]/30 hover:text-[#f2f3f5] hover:border-[#3a3f4a]" asChild disabled={uploadingBilan}>
                <span>
          ...
          {(formData.bilans_locataire || []).map((bilan, idx) => (
            <div key={idx} className="flex items-center gap-3 p-3 bg-[#f2f3f5]/[0.02] rounded-lg border border-[#1f2228]">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[#f2f3f5] text-sm truncate">{bilan.nom}</p>
              </div>
          ...
          {(!formData.bilans_locataire || formData.bilans_locataire.length === 0) && (
            <p className="text-[#9298a6] text-sm text-center py-2">Aucun bilan importé.</p>
          )}
        </div>
        ...
        <div className="space-y-4 pt-4 border-t border-[#1f2228]">
          <div className="flex items-center justify-between">
            <Label className="text-[#f2f3f5]">Notes locataire</Label>
            <Button
              type="button"
              variant="outline"
              size="sm"
