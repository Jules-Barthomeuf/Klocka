...
export default function Questionnaire() {
  ...
  if (showResult && profil) {
    ...
    return (
      <div className="min-h-screen bg-fond flex items-center justify-center p-4">
        <Card className="max-w-2xl w-full border-menthe/30 bg-gradient-to-br from-fond to-black shadow-2xl">
          <CardHeader className="pb-6 border-b border-surface">
            <div className="flex items-center gap-4 mb-4">
              <div className="flex-1 text-center">
                <div className="w-12 h-12 mx-auto mb-2 bg-menthe rounded-full flex items-center justify-center">
                  <CheckCircle2 className="w-6 h-6 text-encre" />
                </div>
                <CardTitle className="text-2xl max-md:text-xl font-montserrat text-encre">
          ...
          <CardContent className="p-8">
            <div className="text-center space-y-6">
              <img 
                ...
                alt={profilInfo.nom}
                className="w-64 h-64 max-md:w-48 max-md:h-48 mx-auto object-cover rounded-md shadow-lg border-2 border-menthe"
              />

              <h2 className="text-2xl max-md:text-xl text-menthe">
                {profilInfo.nom}
              </h2>

              <p className="text-craie leading-relaxed text-lg max-md:text-base">
  ...
  return (
    <div className="min-h-screen bg-fond flex items-center justify-center p-4 max-md:p-3">
      <Card className="max-w-3xl w-full border-menthe/30 bg-gradient-to-br from-fond to-black shadow-2xl">
        <CardHeader className="space-y-4 pb-8 max-md:pb-6 max-md:space-y-3 border-b border-surface">
          <div className="flex justify-between items-center max-md:flex-col max-md:items-start max-md:gap-2">
            <CardTitle className="text-3xl max-md:text-xl text-encre">
              {currentQuestion === 0 ? "Bienvenue chez Klocka !" : "Questionnaire initial"}
            </CardTitle>
            <div className="text-sm max-md:text-xs font-medium text-ardoise">
              Question {questionNumber} / {totalQuestions}
            </div>
          </div>
          <Progress value={progress} className="h-2 max-md:h-1.5 bg-surface" />
        </CardHeader>
        
        <CardContent className="space-y-8 max-md:space-y-6 p-8 max-md:p-4">
          <div className="space-y-6 max-md:space-y-4">
            <h2 className="text-xl max-md:text-base font-semibold text-encre leading-relaxed">
              {question.question}
            </h2>
            ...
            {question.type === "text" ? (
              <div className="space-y-6">
                <Input
                  ...
                  className="h-12 max-md:h-10 text-lg max-md:text-base bg-fond border-bord text-encre placeholder:text-ardoise focus:border-menthe focus:ring-menthe"
                  autoFocus
                />
                  ...
                  <div className="p-4 bg-surface/50 rounded-md border border-bord space-y-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        variant="outline"
                        className="border-menthe text-menthe hover:bg-menthe/10"
                      >
              ...
              >
                {question.options.map((option) => (
                  ...
                  >
                    <RadioGroupItem value={option.value} id={option.value} className="mt-1 border-bord" />
                    <Label 
                      htmlFor={option.value} 
                      className="flex-1 cursor-pointer text-base font-medium leading-relaxed text-encre"
                    >
                      {option.label}
                    </Label>
                    {reponses[question.id] === option.value && (
          ...
          <div className="flex justify-between gap-4 max-md:gap-2 pt-6 max-md:pt-4 border-t border-surface">
            <Button
              variant="outline"
              onClick={handlePrevious}
              disabled={currentQuestion === 0}
              className="px-8 max-md:px-4 max-md:text-sm border-bord text-encre hover:bg-surface"
            >
              Précédent
            </Button>
