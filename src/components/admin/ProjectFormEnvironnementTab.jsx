  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center gap-1">
        <Label className="text-ardoise text-xs">{label}</Label>
        {info && (
          <div className="relative">
            <button
              type="button"
              onClick={() => setShow(v => !v)}
              className="text-brume hover:text-encre transition-colors"
            >
              <Info className="w-3 h-3" />
            </button>
            {show && (
              <div className="absolute z-50 bottom-full left-1/2 -translate-x-1/2 mb-2 w-52 bg-[#0c0d10] border border-trait rounded-lg p-2.5 text-xs text-craie shadow-xl">
                {info}
                <div className="absolute top-full left-1/2 -translate-x-1/2 w-2 h-2 bg-[#0c0d10] border-r border-b border-trait rotate-45 -mt-1" />
              </div>
            )}
          </div>
        )}
...
export default function ProjectFormEnvironnementTab({ formData, setFormData }) {
  ...
  return (
    <div className="space-y-8 mt-4">
      ...
      {/* DÉMOGRAPHIE */}
      <div>
        <h3 className="text-sm font-semibold text-ardoise uppercase tracking-wider mb-3">Démographie</h3>
      ...
      <div>
        <h3 className="text-sm font-semibold text-ardoise uppercase tracking-wider mb-3">Politique de la ville</h3>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-3">
          <Field label="Stabilité politique" info="Stabilité de la gouvernance municipale : ancienneté du maire, majorité confortable, renouvellements récents…">
            <Select value={env.stabilite_politique || "__none__"} onValueChange={(v) => update("stabilite_politique", v === "__none__" ? null : v)}>
              <SelectTrigger className="bg-encre/[0.03] text-encre border-trait h-8 text-sm"><SelectValue placeholder="—" /></SelectTrigger>
              <SelectContent>
      ...
      {/* PROJETS URBAINS */}
      <div>
        <h3 className="text-sm font-semibold text-ardoise uppercase tracking-wider mb-3">Projets urbains & Aménagements</h3>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-3">
      ...
      {/* ATTRACTIVITÉ */}
      <div>
        <h3 className="text-sm font-semibold text-ardoise uppercase tracking-wider mb-3">Attractivité & Rayonnement</h3>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-3">
      ...
      {/* ÉCONOMIE LOCALE */}
      <div>
        <h3 className="text-sm font-semibold text-ardoise uppercase tracking-wider mb-3">Économie locale</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
