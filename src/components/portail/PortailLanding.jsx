  ...
  return (
    <div className="text-center">
      <button onClick={onClick}
        className="inline-flex items-center gap-2.5 bg-[#35a79b] hover:bg-[#238B7F] text-[#edeae5] font-bold px-10 py-4 rounded-md transition-all text-base group shadow-lg shadow-[#35a79b]/20">
        {label} <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
      </button>
      <p className="text-[#6b7270] text-xs mt-3 italic">Places limitées par cohorte</p>
    </div>
  );
}
...
function Section({ children, id, dark = true }) {
  return (
    <section id={id} className={dark ? "bg-[#0a0c0c] text-[#edeae5]" : "bg-[#171918] text-[#edeae5]"}>
...
function Label({ children }) {
  return <p className="uppercase tracking-[0.25em] text-[11px] font-semibold mb-5 text-center text-[#35a79b]">{children}</p>;
}

function Title({ children }) {
  return <h2 className="text-2xl md:text-4xl font-extrabold mb-4 text-center leading-tight uppercase tracking-wide text-[#edeae5]">{children}</h2>;
}

function Sub({ children }) {
  return <p className="text-sm md:text-base text-center max-w-2xl mx-auto mb-10 leading-relaxed text-[#9aa19e]">{children}</p>;
}
...
export default function PortailLanding({ onContinue }) {
  ...
  return (
    <div>
      ...
      {/* ═══════ SECTION 1 — HERO ═══════ */}
      <section className="bg-[#0a0c0c] text-[#edeae5]">
        <div className="max-w-6xl mx-auto px-6 py-16 md:py-28">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            {/* Left — Text */}
            <div>
              <div className="inline-flex items-center gap-2 bg-[#35a79b]/10 border border-[#35a79b]/30 rounded-full px-5 py-2 mb-8">
                <span className="w-2 h-2 rounded-full bg-[#35a79b] animate-pulse" />
                <span className="text-[#35a79b] text-xs font-semibold tracking-wide">3 deals sourcés inclus dans l'accompagnement</span>
              </div>

              <h1 className="text-3xl md:text-4xl lg:text-5xl font-black text-[#edeae5] leading-[1.08] mb-6 uppercase tracking-tight">
                Votre premier investissement en immobilier commercial,{" "}
                <span className="text-[#35a79b]">réalisé pour vous</span>, depuis votre canapé.
              </h1>

              <p className="text-[#9aa19e] text-base md:text-lg leading-relaxed mb-8">
                Klocka ne vous forme pas. Klocka investit avec vous : sourcing, analyse, financement, gestion. <strong className="text-[#edeae5]">On s'occupe de tout.</strong>
              </p>

              <div className="grid grid-cols-2 gap-4 mb-10">
                {STATS.map((s, i) => (
                  <div key={i} className="bg-[#edeae5]/[0.03] border border-[#242726] rounded-md px-4 py-3 text-center">
                    <p className="text-2xl md:text-3xl font-black text-[#edeae5]">{s.value}</p>
                    <p className="text-[#8b9391] text-[10px] mt-0.5 uppercase tracking-wider">{s.label}</p>
                    {s.note && <p className="text-[#6b7270] text-[9px] mt-1 italic normal-case">{s.note}</p>}
                  </div>
                ))}
              </div>

              <button onClick={onContinue}
                className="inline-flex items-center gap-2.5 bg-[#35a79b] hover:bg-[#238B7F] text-[#edeae5] font-bold px-8 py-4 rounded-md transition-all text-base group shadow-lg shadow-[#35a79b]/20">
            ...
            <div className="relative">
              <div className="rounded-md overflow-hidden border border-[#282b2a] shadow-2xl">
      ...
      <section className="bg-[#0a0c0c] text-[#edeae5]">
        ...
        <p className="uppercase tracking-[0.25em] text-[11px] font-semibold mb-5 text-center text-[#35a79b]">Le constat</p>
        <h2 className="text-3xl md:text-5xl font-extrabold mb-4 text-center leading-tight uppercase tracking-wide text-[#edeae5]">
          Vous savez que l'immobilier commercial est un des meilleurs placements. <span className="text-[#35a79b]">Mais vous n'avez jamais franchi le cap.</span>
        </h2>
        <p className="text-base md:text-lg text-center max-w-2xl mx-auto mb-10 leading-relaxed text-[#9aa19e]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 max-w-4xl mx-auto mb-14">
          {PAINS.map((p, i) => (
            <div key={i} className="relative bg-[#edeae5]/[0.04] border border-[#282b2a] rounded-md px-6 py-8 text-center hover:border-red-500/30 hover:shadow-lg transition-all group">
              ...
              <p className="text-[#edeae5] text-lg font-bold leading-snug">{p}</p>
            </div>
          ))}
        </div>

        <div className="max-w-3xl mx-auto mb-12 text-center">
          <p className="text-[#edeae5] text-2xl md:text-3xl font-extrabold leading-tight mb-2">
            Vous n'avez pas à tout faire seul.
          </p>
          <p className="text-[#edeae5] text-xl md:text-2xl font-semibold">
            C'est exactement pour cela que <span className="text-[#35a79b] font-bold">Klocka existe.</span>
          </p>
        </div>
      ...
      <Section dark>
        ...
        <Title>
          Klocka, ce n'est pas une formation. <span className="text-[#35a79b]">C'est votre Deal Maker.</span>
        </Title>
        <Sub>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0 max-w-4xl mx-auto mb-12 rounded-md overflow-hidden">
          ...
          {/* Klocka - Green */}
          <div className="bg-[#35a79b]/10 border border-[#35a79b]/25 p-6 md:p-8 md:rounded-r-2xl">
            <div className="flex items-center gap-2 mb-6">
              <Check className="w-5 h-5 text-[#35a79b]" />
              <p className="text-[#35a79b] text-xs font-bold uppercase tracking-wider">Klocka — Clé en Main</p>
            </div>
            <div className="space-y-4">
              {COMPARE_KLOCKA.map((item, i) => (
                <div key={i} className="flex items-start gap-3">
                  <Check className="w-4 h-4 text-[#35a79b] mt-0.5 flex-shrink-0" />
                  <p className="text-[#edeae5] text-sm font-medium">{item}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-[#edeae5]/[0.03] border border-[#242726] rounded-md px-6 py-5 max-w-2xl mx-auto text-center mb-12">
          <p className="text-[#edeae5] text-base">
            « Vous apportez votre capacité d'investissement. <span className="text-[#35a79b] font-bold">Nous apportons tout le reste.</span> »
          </p>
        </div>
      ...
      <section className="bg-[#0a0c0c] text-[#edeae5]">
        <div className="max-w-6xl mx-auto px-6 py-20 md:py-28">
          <p className="uppercase tracking-[0.25em] text-[11px] font-semibold mb-4 text-center text-[#35a79b]">L'accompagnement</p>
          <h2 className="text-3xl md:text-5xl font-black mb-4 text-center leading-tight uppercase tracking-wide text-[#edeae5]">
            Un service complet, <span className="text-[#35a79b]">de A à Z</span>.
          </h2>
          <p className="text-base md:text-lg text-center max-w-2xl mx-auto mb-14 leading-relaxed text-[#9aa19e]">Chaque module montre concrètement que vous n'avez rien à faire de plus que choisir votre deal.</p>

          {/* ── OFFRE SOCLE (1-4) ── */}
          <div className="mb-6">
            <div className="inline-flex items-center gap-2 bg-[#35a79b]/10 border border-[#35a79b]/30 rounded-full px-5 py-2 mb-6">
              <span className="w-2 h-2 rounded-full bg-[#35a79b]" />
              <span className="text-[#35a79b] text-xs font-bold uppercase tracking-wider">Offre socle — inclus dans l'accompagnement</span>
            </div>
              ...
              {SERVICES.slice(0, 4).map((s) => {
                ...
                return (
                  <div key={s.num} className={`rounded-md border transition-all hover:shadow-lg relative overflow-hidden ${
                    isHighlight
                      ? 'bg-[#0a0c0c] border-[#e0c9a0]/50 p-0 shadow-[0_0_30px_rgba(251,191,36,0.08)]'
                      : 'bg-[#0a0c0c] border-[#171918] hover:border-[#343735] p-6'
                  }`}>
                    {isHighlight ? (
                      <div className="p-6 md:p-8 relative">
                        ...
                        <div className="flex flex-col md:flex-row md:items-center gap-6">
                          <div className="flex-shrink-0">
                            <div className="w-16 h-16 rounded-md bg-[#e0c9a0]/10 border border-[#e0c9a0]/20 flex items-center justify-center">
                              <Icon className="w-8 h-8 text-[#e0c9a0]" />
                            </div>
                          </div>
                          <div className="flex-1">
                            <span className="text-[#e0c9a0]/60 text-xs font-mono font-bold block mb-1">Étape {s.num}</span>
                            <h3 className="text-[#edeae5] text-xl md:text-2xl font-black leading-tight mb-3">{s.title}</h3>
                            <ul className="space-y-2">
                              {s.items.map((item, j) => (
                                <li key={j} className="flex items-start gap-2.5 text-base text-[#d3d8d6] leading-relaxed">
                                  <Check className="w-5 h-5 flex-shrink-0 mt-0.5 text-[#e0c9a0]" />
                                  {item}
                                </li>
                              ))}
                            </ul>
                      ...
                      <>
                        <div className="flex items-center gap-4 mb-4">
                          <div className="w-12 h-12 rounded-md bg-[#171918] flex items-center justify-center flex-shrink-0">
                            <Icon className="w-6 h-6 text-[#edeae5]" />
                          </div>
                          <div>
                            <span className="text-[#8b9391] text-xs font-mono font-bold block">Étape {s.num}</span>
                            <h3 className="text-[#edeae5] text-lg font-bold leading-tight">{s.title}</h3>
                          </div>
                        </div>
                        <ul className="space-y-2.5 pl-1">
                          {s.items.map((item, j) => (
                            <li key={j} className="flex items-start gap-2.5 text-sm text-[#9aa19e] leading-relaxed">
                              <Check className="w-4 h-4 flex-shrink-0 mt-0.5 text-[#8b9391]" />
                              {item}
                            </li>
                          ))}
                        </ul>
          ...
          <div className="mt-12 mb-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {SERVICES.slice(4).map((s) => {
                const Icon = s.icon;
                return (
                  <div key={s.num} className="rounded-md p-6 border bg-[#0a0c0c] border-[#171918] hover:border-[#343735] transition-all hover:shadow-lg relative overflow-hidden">
                    <div className="absolute top-3 right-3">
                      <span className="text-[10px] font-bold uppercase tracking-wider text-[#e0c9a0] bg-[#e0c9a0]/10 border border-[#e0c9a0]/20 rounded-full px-2.5 py-1">Option</span>
                    </div>
                    <div className="w-12 h-12 rounded-md bg-[#171918] flex items-center justify-center mb-4">
                      <Icon className="w-6 h-6 text-[#edeae5]" />
                    </div>
                    <span className="text-[#8b9391] text-xs font-mono font-bold block">Étape {s.num}</span>
                    <h3 className="text-[#edeae5] text-lg font-bold leading-tight mb-3 mt-1">{s.title}</h3>
      ...
      <Section dark>
        ...
        <Title>
          Investissez depuis n'importe où. <span className="text-[#35a79b]">Tout est centralisé.</span>
        </Title>
        <Sub>Vous recevez des opportunités d'investissement analysées. Pas des devoirs à rendre.</Sub>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl mx-auto mb-12">
          {PLATFORM.map((f, i) => (
            <div key={i} className="flex items-center gap-4 bg-[#edeae5]/[0.04] border border-[#282b2a] rounded-md px-5 py-4">
              <div className="w-10 h-10 rounded-md bg-[#35a79b]/10 flex items-center justify-center flex-shrink-0">
                <f.icon className="w-5 h-5 text-[#35a79b]" />
              </div>
              <p className="text-[#edeae5] text-sm font-medium">{f.label}</p>
            </div>
          ))}
        </div>

        {/* Platform screenshot */}
        <div className="max-w-3xl mx-auto rounded-md overflow-hidden border border-[#282b2a] bg-gradient-to-br from-[#0a0c0c] to-[#0a0c0c] p-3 mb-12">
          <div className="rounded-md bg-[#0a0c0c] border border-[#242726] overflow-hidden">
            <div className="h-7 bg-[#0a0c0c] flex items-center gap-1.5 px-3 border-b border-[#242726]">
              <div className="w-2.5 h-2.5 rounded-full bg-red-500/60" />
              <div className="w-2.5 h-2.5 rounded-full bg-[#e0c9a0]/60" />
              <div className="w-2.5 h-2.5 rounded-full bg-[#35a79b]/60" />
            </div>
      ...
      <Section dark={false}>
        ...
        <Title>
          Ils ont investi avec <span className="text-[#35a79b]">Klocka</span>.
        </Title>

        {/* Video témoignage */}
        <div className="max-w-3xl mx-auto rounded-md overflow-hidden border border-[#282b2a] bg-[#0a0c0c] aspect-video mb-8">
          <iframe
      ...
      <Section dark>
        ...
        <Title>
          Deux experts au service de <span className="text-[#35a79b]">votre investissement.</span>
        </Title>
        <Sub>
        ...
        <div className="space-y-6 max-w-3xl mx-auto mb-12">
          {FOUNDERS.map((f, i) => (
            <div key={i} className="bg-[#edeae5]/[0.03] border border-[#282b2a] rounded-md p-6 md:p-8 flex flex-col md:flex-row items-center md:items-start gap-6 hover:border-[#edeae5]/[0.14] transition-colors">
              <img src={f.photo} alt={f.name} className="w-28 h-28 rounded-full object-cover flex-shrink-0 border-2 border-[#35a79b]/20" />
              <div className="text-center md:text-left">
                <p className="text-[#edeae5] font-bold text-lg mb-0.5">{f.name}</p>
                <p className="text-[#35a79b] text-xs font-semibold uppercase tracking-wider mb-3">{f.role}</p>
                <p className="text-[#9aa19e] text-sm leading-relaxed">{f.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="bg-[#0a0c0c] text-[#edeae5]">
        <div className="max-w-5xl mx-auto px-6 py-20 md:py-28">
          <div className="bg-gradient-to-br from-[#35a79b]/15 via-[#35a79b]/5 to-transparent border border-[#35a79b]/25 rounded-3xl p-8 md:p-14 text-center max-w-3xl mx-auto">
            <img src={LOGO_KLOCKA} alt="Klocka" className="h-10 mx-auto mb-6 object-contain" />
            <Label>Dernière étape</Label>
            <h2 className="text-2xl md:text-4xl font-black text-[#edeae5] mb-4 uppercase tracking-wide">
              Prêt à investir ?
            </h2>
            <p className="text-[#9aa19e] text-sm md:text-base mb-8 max-w-lg mx-auto leading-relaxed">
              Pour finaliser votre inscription, consultez et acceptez nos conditions générales, puis procédez au paiement sécurisé.
            </p>
            <button onClick={onContinue}
              className="inline-flex items-center gap-2.5 bg-[#35a79b] hover:bg-[#238B7F] text-[#edeae5] font-bold px-10 py-4 rounded-md transition-all text-base group shadow-lg shadow-[#35a79b]/25">
              Démarrer l'accompagnement <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
            </button>
            <p className="text-[#6b7270] text-xs mt-4 italic">Places limitées par cohorte</p>
          </div>
        </div>
      </section>

      {/* ═══════ FOOTER ═══════ */}
      <footer className="bg-[#0a0c0c] border-t border-[#232625] px-6 py-8">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-6">
            <img src={LOGO_KLOCKA} alt="Klocka" className="h-5 w-auto opacity-60" />
            <p className="text-[#6b7270] text-xs">© 2025 Klocka · Développeur de revenus immobiliers</p>
          </div>
          <div className="flex items-center gap-6">
