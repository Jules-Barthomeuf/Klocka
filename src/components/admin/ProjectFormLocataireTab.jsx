      ...
      <div className="space-y-4">
        ...
        <div className="p-4 bg-encre/[0.02] rounded-lg space-y-4">
          <div className="flex items-center gap-3">
            ...
            />
            <Label className="text-encre">Vente récente de fonds de commerce</Label>
          </div>
        ...
        <div className="space-y-4 pt-4 border-t border-trait">
          <div className="flex items-center justify-between">
            <div>
              <Label className="text-encre text-lg">Champs personnalisés du résumé</Label>
          ...
          {(!formData.bail_admin_fields || formData.bail_admin_fields.length === 0) && (
            <p className="text-ardoise text-sm text-center py-2">Aucun champ personnalisé. Cliquez sur le bouton ci-dessus pour en ajouter.</p>
          )}
        </div>
        ...
        <div className="space-y-4 pt-4 border-t border-trait">
          <div className="flex items-center justify-between">
            <div>
              <Label className="text-encre text-lg">Réseaux sociaux & Avis</Label>
              <p className="text-ardoise text-sm mt-1">Ajoutez les liens vers les réseaux sociaux et pages d'avis du locataire</p>
            </div>
            <Button
              type="button"
              variant="outline"
          ...
          {(formData.liens_locataire || []).map((lien, idx) => (
            <div key={idx} className="p-4 bg-encre/[0.02] rounded-lg space-y-3">
              <div className="flex items-center gap-3">
                ...
                >
                  <SelectTrigger className="bg-surface border border-trait rounded-none text-encre w-40 h-[52px] px-[18px]">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
          ...
          {(!formData.liens_locataire || formData.liens_locataire.length === 0) && (
            <p className="text-ardoise text-sm text-center py-2">Aucun lien ajouté.</p>
          )}
        </div>
        ...
        <div className="space-y-4 pt-4 border-t border-trait">
          <div className="flex items-center justify-between">
            <div>
              <Label className="text-encre text-lg">Bilans du locataire</Label>
              <p className="text-ardoise text-sm mt-1">Importez les bilans financiers de l'entreprise (PDF, images...)</p>
            </div>
            <label className="cursor-pointer">
              <input type="file" className="hidden" onChange={handleBilanUpload} accept=".pdf,.png,.jpg,.jpeg,.xlsx,.xls" />
              <Button type="button" variant="outline" size="sm" className="border-trait text-encre/30 hover:text-encre hover:border-bord-vif" asChild disabled={uploadingBilan}>
                <span>
          ...
          {(formData.bilans_locataire || []).map((bilan, idx) => (
            <div key={idx} className="flex items-center gap-3 p-3 bg-encre/[0.02] rounded-lg border border-trait">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-encre text-sm truncate">{bilan.nom}</p>
              </div>
          ...
          {(!formData.bilans_locataire || formData.bilans_locataire.length === 0) && (
            <p className="text-ardoise text-sm text-center py-2">Aucun bilan importé.</p>
          )}
        </div>
        ...
        <div className="space-y-4 pt-4 border-t border-trait">
          <div className="flex items-center justify-between">
            <Label className="text-encre">Notes locataire</Label>
            <Button
              type="button"
              variant="outline"
              size="sm"
