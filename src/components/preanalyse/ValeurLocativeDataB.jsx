...
function Niveau({ titre, n, loyerM2 }) {
  if (!n) return null;
  const dedans = loyerM2 != null && n.basse != null && n.haute != null && loyerM2 >= n.basse && loyerM2 <= n.haute;
  return (
    <div className="flex items-baseline justify-between gap-4 py-2.5 border-b border-[#15171b]">
      <div className="min-w-0">
        ...
        <p className="m-0 text-[14px] text-[#f2f3f5] truncate">{n.nom || "—"}</p>
      </div>
      <p className={`m-0 text-[15px] tabular-nums font-light whitespace-nowrap ${dedans ? "text-[#d9b46a]" : "text-[#f2f3f5]"}`}>
        {euros(n.basse)} <span className="text-[#3a3f4a]">–</span> {euros(n.haute)}
      </p>
    </div>
  );
}
...
export default function ValeurLocativeDataB({ dossier, lot, apercu = false, onRefresh }) {
  ...
  const chercher = useMutation({
    ...
    onSuccess: (r) => {
      ...
      if (r.resultat?.du_cache) toast.message("Valeur locative déjà connue", { description: "Résultat gardé de moins de trente jours. « Relire » interroge Data-B de nouveau." });
      onRefresh?.();
    },
