      ...
      <div className="overflow-x-auto border border-[#1e1e22]">
        <table className="border-collapse text-[12px] min-w-full">
          <thead><tr className="bg-[#0a0a0b]">
            ...
            {g.colonnes.map((c) => <th key={c.id} title={c.question} className="text-left px-3 py-2 text-[11.5px] font-semibold text-encre border-b border-r border-[#1e1e22] min-w-[170px]">{c.libelle}</th>)}
          </tr></thead>
          <tbody>
            {g.lignes.map((l) => (
              <tr key={l.document_id} className={l.perime ? "opacity-50" : ""}>
                <td className="sticky left-0 bg-fond px-3 py-2 border-b border-r border-[#1e1e22] align-top">
      ...
      <div className="px-6 py-8">
        ...
        <div className="mt-5 flex items-center gap-4">
          {enCours ? <span className="inline-flex items-center gap-2 text-[13px] text-ardoise"><Loader2 className="w-4 h-4 animate-spin" /> {e.remplissage.fait}/{e.remplissage.total ?? "…"} — {e.remplissage.document || "lecture"}</span>
            : e.progression.presents_etape ? <button onClick={() => lancer.mutate(2)} disabled={apercu || lancer.isPending} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-[10px] bg-encre text-[#0b0c0e] text-[13px] font-semibold hover:bg-[#ffffff] disabled:opacity-40">Lire l'immeuble et la copropriété</button> : null}
        </div>
      </div>
    );
  }
      ...
      <div className="px-6 max-md:px-4 flex items-center justify-between gap-4 border-b border-trait">
        <div className="flex gap-6">
          {[["blocs", "Synthèse"], ["grilles", "Extraction"]].map(([id, l]) => (
            <button key={id} onClick={() => setVue(id)} className={`relative py-3 text-[14px] after:absolute after:left-0 after:right-0 after:-bottom-px after:h-[2px] after:bg-encre after:origin-left after:scale-x-0 after:transition-transform ${vue === id ? "text-encre font-semibold after:scale-x-100" : "text-[#77777e] hover:text-[#c6ccd3]"}`}>{l}</button>
          ))}
        </div>
        ...
        <>
          <Section id="copro" titre="1 · Copropriété">
            {questions.map((q) => (
              <div key={q.question} className="grid grid-cols-[minmax(220px,1fr)_2fr_130px] max-md:grid-cols-1 gap-x-6 gap-y-1 py-3 border-t border-[#15171b] first:border-t-0 items-baseline">
                ...
                <p className="m-0 text-[14px] leading-[1.55] text-[#d6d6db]"><span className="font-semibold text-encre">{q.tete}</span> — {q.reponse} <Source s={q.source} onPreuve={onPreuve} /></p>
          ...
          <Section id="marche" titre="4 · Marché">
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-x-8 gap-y-5">
              {marche.kpis.map((k) => (
                <div key={k.libelle}><Mono>{k.libelle}</Mono><p className={`m-0 mt-2 text-[20px] font-light tabular-nums ${k.valeur ? "text-encre" : "text-[#4d545d]"}`}>{k.valeur || "—"}</p><p className="m-0 mt-1 text-[12px] text-brume">{k.detail}</p></div>
              ))}
            </div>
