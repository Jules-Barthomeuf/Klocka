...
export default function IndicateursPerformance({ calculs, anneeRevente, formatCurrency, textClass, mutedClass }) {
  return (
    <div className="bg-[#050807] p-4 rounded-md md:p-6 border border-white/[0.1] max-w-full overflow-hidden">
      <h3 className="font-light text-white text-xl md:text-2xl tracking-tight mb-3 md:mb-4">Performance globale</h3>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4 w-full">
        ...
        <div className="p-2 md:p-4 rounded-md bg-white/[0.04] border border-[#1c2725] flex flex-col items-center justify-center text-center">
          ...
          <p className="text-xs text-white/50 mt-1">soit {formatCurrency(Math.round(calculs.indicateurs.creationRichesseBrute / anneeRevente / 12))}/mois</p>
        </div>
      </div>
    </div>
  );
