...
//     une décote sur les loyers Data-B d'une ville qu'ils surestiment, un
//     plafond de classe pour les rues qui ressemblent à celles qu'on a
//     rétrogradées (résidentielles, trop courtes, sans passage) ;
...
import { emplacementParLoyer, libelleEmplacement } from './rues.js';

export const MOTIFS = [
  { cle: 'loyer_surestime', mot: 'Loyer Data-B trop haut pour cette rue', portee: 'ville', detail: 'Les loyers de cette ville seront lus avec une décote.' },
 ...
 * - une décote sur les loyers d'une ville, quand au moins deux rues y ont
 *   été rétrogradées pour « loyer trop haut » : la décote médiane qui aurait
 *   fait tomber chacune dans la classe voulue ;
 ...
 */
export function reglesApprises(lecons, seuils = {}) {
  const s1 = seuils.loyer_emplacement_1 ?? 800, s15 = seuils.loyer_emplacement_1bis ?? 550, s2 = seuils.loyer_emplacement_2 ?? 250;
  const seuilDe = (classe) => (classe === 1 ? s1 : classe === 1.5 ? s15 : s2);
  const decotes = {};
  const plafonds = {};
  const planchers = [];
  for (const l of lecons) {
    const t = l.traits || {};
    if (l.motif_cle === 'loyer_surestime' && l.vers > l.de && t.loyer_milieu) {
      // Pour tomber sous le seuil de la classe qu'ALX avait donnée, il faut ramener le loyer juste sous ce seuil.
      const cible = seuilDe(l.de) - 1;
      const d = Math.min(0.4, Math.max(0, 1 - cible / t.loyer_milieu));
      (decotes[l.ville_id] = decotes[l.ville_id] || []).push(d);
    } else if (['peu_de_passage', 'residentielle', 'trop_courte'].includes(l.motif_cle) && l.vers > l.de) {
  ...
  return {
    decotes: Object.fromEntries(Object.entries(decotes).filter(([, ds]) => ds.length >= 2).map(([v, ds]) => [v, Math.round(mediane(ds) * 100) / 100])),
 ...
 */
export function appliquerLecons(rue, regles, villeId, seuils) {
  let r = { ...rue };
  const notes = [];
  const decote = regles.decotes?.[villeId];
  if (decote && r.loyer) {
    const loyerCorrige = { basse: r.loyer[0] * (1 - decote), haute: r.loyer[1] * (1 - decote) };
    const e = emplacementParLoyer(loyerCorrige, r.commerces || 0, seuils);
    if (e.classe && e.classe !== r.classe) { r.classe = e.classe; notes.push(`loyer Data-B lu avec ${Math.round(decote * 100)} % de décote, d'après vos corrections`); }
  }
  const t = traitsDe(r);
