...
function Source({ s, onPreuve, texte }) {
  ...
  return <button onClick={() => onPreuve(s)} className="font-mono text-[11px] tracking-[.06em] text-brume hover:text-encre whitespace-nowrap">{(s.document_nom || "").replace(/\.pdf$/i, "").slice(0, 28)}{s.page ? ` p. ${s.page}` : ""}</button>;
}
  ...
  return (
    <>
          ...
          <tbody>
            {postes.map(([l, t, d, src, note, u]) => (
              <tr key={l} className="border-t border-relief">
                <td className="py-3 text-[13.5px] text-craie">{l}</td>
                <td className="py-3 pr-6 text-right tabular-nums font-light text-[15px] text-brume">{fmt(t, u)}</td>
                <td className="py-3 text-right tabular-nums font-light text-[13.5px] text-encre">{fmt(d, u)}</td>
                <td className="py-3 pl-5"><Source s={src} onPreuve={onPreuve} texte={note} /></td>
              </tr>
            ))}
          </tbody>
        </table>
        <p className="m-0 mt-3 text-[12.5px] leading-[1.6] text-ardoise max-w-[860px]">{r.phrase}</p>
      </Section>
      ...
      <Section id="ecarts" titre="Écarts avec la pré-analyse" droite={e.ecarts.length ? `${e.ecarts.length} champ${e.ecarts.length > 1 ? "s" : ""} qui bouge${e.ecarts.length > 1 ? "nt" : ""}` : null}>
            ...
            {e.ecarts.map((x) => {
              ...
              return (
                <div key={x.libelle} className="grid grid-cols-[130px_minmax(0,1fr)_140px] max-md:grid-cols-1 gap-x-6 gap-y-1 py-4 items-baseline">
                  <span className="text-[12.5px] text-brume">{x.libelle}</span>
                  <div className="min-w-0">
                    <div className="flex items-baseline gap-x-3 min-w-0">
                      ...
                      <span className="text-[18px] font-light tabular-nums text-encre truncate" title={String(x.bail)}>{x.bail}</span>
                    </div>
                    <p className="m-0 mt-1 text-[12.5px] leading-[1.6] text-ardoise max-w-[760px]">{x.commentaire}</p>
                  </div>
        ...
        ) : <p className="m-0 text-[12.5px] text-ardoise">Aucun écart significatif avec la pré-analyse.</p>}
      </Section>
          ...
          {fiche.lignes.map((l) => (
            <React.Fragment key={l.id}>
              ...
              <dd className="m-0 min-w-0">
                {l.source ? <button onClick={() => onPreuve(l.source)} className="text-left text-[13.5px] leading-[1.5] text-encre hover:text-[#ffffff]">{l.valeur}</button> : <span className="text-[13.5px] text-encre">{l.valeur}</span>}
      ...
      <Section id="pieces" titre="Documents pour l'étape 2" droite={`${d2.importes} importé${d2.importes > 1 ? "s" : ""} · ${d2.manquants.length} manquant${d2.manquants.length > 1 ? "s" : ""}`} sansFilet>
        {d2.presents.length > 0 && <p className="m-0 text-[12.5px] text-craie"><span className="text-brume">Présents : </span>{d2.presents.map((p) => `${p.categorie} (${p.n})`).join(" · ")}</p>}
        {d2.manquants.length > 0 && <ul className="m-0 mt-2 pl-4 space-y-0.5">{d2.manquants.map((x) => <li key={x.piece} className="text-[12.5px] text-craie">{x.piece} — <span className="text-brume">{x.detail}</span></li>)}</ul>}
      </Section>
