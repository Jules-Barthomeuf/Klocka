...
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[7]
    ...
    // Brand color mapping - #2A9D8F (teal) and #71CCBA (light teal)
    vec3 color1 = vec3(0.165, 0.616, 0.561); // #2A9D8F
    vec3 color2 = vec3(0.443, 0.8, 0.729);    // #71CCBA
    vec3 color3 = vec3(0.02, 0.02, 0.02);     // Near black
    
    // Mix colors based on the output values
    vec3 finalColor = mix(color3, color1, buf[0].x);
    finalColor = mix(finalColor, color2, buf[0].y * 0.6);
...
// Extend immediately after creation
...
function ShaderPlane() {
  ...
  useFrame((state) => {
    ...
    try {
      ...
      console.warn('Shader frame error:', error);
    }
  });
...
export function CPPNShaderAnimation() {
  const [hasError, setHasError] = useState(false);
  const camera = useMemo(() => ({ position: [0, 0, 1], fov: 75, near: 0.1, far: 1000 }), []);
  
  if (hasError) {
    return (
      <div className="absolute inset-0 -z-10 w-full h-full bg-gradient-to-br from-[#2A9D8F]/10 to-black" aria-hidden="true" />
    );
  }
