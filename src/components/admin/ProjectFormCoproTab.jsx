...
export default function ProjectFormCoproTab({ formData, setFormData }) {
  return (
    <div className="space-y-6 mt-6">
      <h3 className="text-lg mb-4 text-encre">Copropriété</h3>
      <div className="space-y-4">
        ...
        <div className="space-y-4 pt-4 border-t border-trait">
          <div className="flex items-center justify-between">
            <Label className="text-encre">Assemblées Générales</Label>
            <Button type="button" variant="outline" size="sm" onClick={() => setFormData({...formData, assemblees_generales: [...(formData.assemblees_generales || []), { annee: new Date().getFullYear(), synthese: "", resolutions_votees: "", resolutions_refusees: "" }]})} className="border-trait text-encre/30 hover:text-encre hover:border-bord-vif">
              <Plus className="w-4 h-4 mr-1" /> Ajouter une AG
            </Button>
          </div>
          {(formData.assemblees_generales || []).map((ag, idx) => (
            <div key={idx} className="p-4 bg-encre/[0.02] rounded-lg space-y-3 border border-trait">
          ...
          {(!formData.assemblees_generales || formData.assemblees_generales.length === 0) && <p className="text-ardoise text-sm text-center py-2">Aucune AG.</p>}
        </div>

        {/* Notes libres */}
        <div className="space-y-4 pt-4 border-t border-trait">
          <div className="flex items-center justify-between">
            <Label className="text-encre">Notes libres</Label>
            <Button type="button" variant="outline" size="sm" onClick={() => setFormData({...formData, notes_libres: [...(formData.notes_libres || []), { titre: "", contenu: "" }]})} className="border-trait text-encre/30 hover:text-encre hover:border-bord-vif">
          ...
          {(!formData.notes_libres || formData.notes_libres.length === 0) && <p className="text-ardoise text-sm text-center py-2">Aucune note.</p>}
        </div>
      </div>
    </div>
  );
