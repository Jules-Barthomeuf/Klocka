...
function Row({ label, value, empty = "—" }) {
  return (
    <div className="flex justify-between text-[13px] text-[#9298a6] py-1.5 border-t border-[#1f2228]">
      <span className="flex-shrink-0">{label}</span>
      <span className="text-[#f2f3f5] truncate max-w-[190px] text-right ml-2">{value || empty}</span>
    </div>
  );
}
...
function Shell({ children, formData }) {
  return (
    <div className="bg-[#12151C] border border-[#1f2228] rounded-[18px] overflow-hidden max-w-[360px] shadow-[0_24px_50px_rgba(0,0,0,0.5)]">
      <div
...
export default function ProjectLivePreview({ activeTab, formData }) {
  ...
  return (
    <div>
      <p className="text-[11px] uppercase tracking-[0.18em] text-[#6a7180] mb-3 font-semibold">{label}</p>
