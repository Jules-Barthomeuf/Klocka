  ...
  return (
    <>
            ...
            <div className="mt-2 space-y-2">
              {[["fiche", "PDF", "Fiche de deal mise en page", "Une à deux pages prêtes à joindre à un mail : résumé du bien, risques retenus, prix recommandé, conditions, pour et contre."], ["mail", "Mail", "Message court", "Les points clés dans le corps du mail, la fiche en pièce jointe. Personnalisé avec les critères de l'investisseur."], ["pitch", "Présentation", "Support de call", "Support pour un call ou une réunion : chiffres clés, risques, prix, pour et contre."]].map(([id, tag, t, d]) => (
                <button key={id} onClick={() => setFormat(id)} className={`w-full text-left px-4 py-3 border transition-colors ${format === id ? "border-menthe/60" : "border-trait hover:border-bord-doux"}`}>
                  <p className="m-0 text-[14px] font-semibold text-encre"><Mono className="mr-2">{tag}</Mono>{t}</p>
      ...
      <div className="px-6 max-md:px-4 py-5 border-b border-trait">
        ...
        ) : <p className="m-0 text-[13px] text-ardoise">Rien encore.</p>}
      </div>
      ...
      <div className="px-6 max-md:px-4 py-5 border-b border-trait">
        ...
        {!e.complements.depuis ? <p className="m-0 text-[13px] text-ardoise">La fiche sera photographiée au passage à l'étape 3 ; les compléments se mesurent contre elle.</p>
          : !e.complements.lignes.length ? <p className="m-0 text-[13px] text-ardoise">{e.complements.pieces.length ? `${e.complements.pieces.join(", ")} — sans changement sur la fiche.` : "Aucun complément depuis l'étape 3."}</p>
          : (
            <table className="w-full border-collapse text-[13px]">
              ...
              <tbody>{e.complements.lignes.map((l) => (
                <tr key={l.champ} className="border-t border-[#15171b]">
                  ...
                  <td className="py-2 pl-3 text-[12px] text-brume">{l.source || ""}</td>
                </tr>
              ))}</tbody>
            </table>
          )}
      ...
      <div className="px-6 max-md:px-4 py-5">
        ...
        {e.conclusion ? (
          <p className="m-0 text-[14px] text-encre">{{ signe: "Signé", perdu: "Perdu", abandonne: "Abandonné" }[e.conclusion.etat]}{e.conclusion.motif ? ` — ${e.conclusion.motif}` : ""} <span className="text-brume text-[12px]">· {new Date(e.conclusion.le).toLocaleString("fr-FR", { dateStyle: "short", timeStyle: "short" })}{e.conclusion.par ? ` · ${e.conclusion.par}` : ""}</span></p>
        ) : (
          <>
