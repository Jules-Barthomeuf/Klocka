...
export default function ShadowEditorDialog({ open, onOpenChange, project, shadowRecord, users, initialTab, initialViewMode }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto bg-fond border-trait" onInteractOutside={(e) => e.preventDefault()}>
        <DialogHeader>
          <div className="flex items-center justify-between">
            ...
            <Button variant="ghost" size="icon" onClick={() => onOpenChange(false)} className="text-ardoise hover:text-encre hover:bg-encre/5">
              <X className="w-5 h-5" />
            </Button>
          </div>
        </DialogHeader>
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-md border text-sm transition-all ${
              viewMode === "current"
                ...
                : "bg-fond border-trait text-ardoise hover:text-encre"
            }`}
          >
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="secteur" className="space-y-6 mt-6">
              <div className="space-y-4">
                ...
                <p className="text-ardoise text-sm">Utilisez les onglets Infos / Locataire / Copro pour les champs principaux.</p>
              </div>
            </TabsContent>
        ...
        {/* Save button */}
        <div className="flex justify-end gap-3 mt-6 pt-6 border-t border-trait">
          <button onClick={() => onOpenChange(false)} className="px-5 py-2.5 text-ardoise hover:text-encre text-sm transition-colors">Annuler</button>
          {viewMode === "shadow" ? (
            <button
            ...
            <button
              ...
              className="flex items-center gap-2 bg-blue-500/15 border border-blue-500/30 hover:bg-blue-500/25 text-encre px-6 py-2.5 rounded-md transition-all text-sm disabled:opacity-40"
            >
