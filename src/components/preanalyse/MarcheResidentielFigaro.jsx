...
function Colonne({ titre, niveau, principal = false }) {
  ...
  return (
    <div className={`border rounded-xl px-4 py-3.5 ${principal ? "border-bord-doux bg-surface" : "border-trait bg-[#0a0a0b]"}`}>
      <p className="m-0 text-[10.5px] tracking-[.18em] uppercase text-brume">{titre}</p>
      <p className="m-0 mt-0.5 text-[14px] text-encre truncate">{niveau.nom || "—"}</p>

      <div className="mt-3 flex items-baseline gap-2 flex-wrap">
        <span className="text-[20px] tabular-nums font-light" style={{ color: teinte }}>{euros(niveau.prix?.median)}</span>
        <span className="text-[12px] text-brume">/ m² à l'achat</span>
      </div>
      ...
      <div className="mt-3 flex items-baseline gap-2 flex-wrap">
        <span className="text-[17px] tabular-nums font-light text-encre">{euros(niveau.loyer?.median)}</span>
        <span className="text-[12px] text-brume">/ m² par mois à la location</span>
      </div>
      <p className="m-0 mt-0.5 text-[11.5px] text-brume tabular-nums">
        {niveau.loyer?.bas != null ? <>{euros(niveau.loyer.bas)} à {euros(niveau.loyer.haut)}<span className="text-bord-vif"> · </span></> : null}
      ...
      {r != null && (
        <p className="m-0 mt-3 pt-3 border-t border-[#15171b] text-[12.5px] text-ardoise">
    ...
    <section className="border border-trait rounded-[16px] bg-[#0a0a0b] px-5 py-4">
      <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
        <div className="flex items-center gap-2">
          <h3 className="m-0 text-[15.5px] font-semibold text-encre">Marché résidentiel</h3>
        ...
        {(resultat?.quartier?.lien || resultat?.lien) && (
          <a href={resultat.quartier?.lien || resultat.lien} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-[12px] text-ardoise hover:text-menthe">
      ...
      {resultat && (
        <div className="mt-4">
          ...
          {rendementCommerce != null && rendementAppart != null && (
            <p className="m-0 mt-3 text-[13px] text-ardoise">
              Le commerce présenté rend{" "}
              <span className="tabular-nums text-encre">{String(rendementCommerce).replace(".", ",")} %</span> brut, contre{" "}
              <span className="tabular-nums text-ambre">{String(rendementAppart).replace(".", ",")} %</span> pour un appartement
