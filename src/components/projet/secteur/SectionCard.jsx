...
export default function SectionCard({ title, children }) {
  return (
    <section className="border-t border-[#f2f3f5]/[0.35] pt-7 max-md:pt-5">
      {title && (
        <div className="text-[10px] tracking-[0.2em] uppercase text-[#aab6f5] mb-5 max-md:mb-4">{title}</div>
      )}
      {children}
    </section>
  );
...
export function ProgressBar({ label, value, color = "#8fa0f2", maxValue = 100 }) {
  return (
    <div>
