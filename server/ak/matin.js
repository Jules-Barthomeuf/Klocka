...
export async function motDuMatin({ mentionner = (x) => x, modele = null, consigne = '' } = {}) {
  ...
  const vide = !m.propositions.length && !m.en_retard.length && !m.a_venir.length && !m.sans_agent.length;
  if (vide) return null;
  ...
  const { text } = await runAgent({
    system: `${consigne}\n\nTu écris le mot du matin dans le groupe de l'équipe : deux à quatre lignes, texte brut, ton de l'équipe (minuscules, « dcp », « bg »), pas de liste à puces, pas de politesse. Tu dis ce qu'il y a à faire aujourd'hui, tu relances nommément ceux qui ont un engagement en retard en recopiant EXACTEMENT la mention entre chevrons telle qu'elle t'est donnée (<users/123>), et tu signales les dossiers sans agent. Rien d'autre, aucun chiffre inventé.`,
