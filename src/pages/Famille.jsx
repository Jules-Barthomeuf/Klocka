    ...
    return (
      <div className="min-h-screen bg-fond p-6 md:p-10">
        <div className="max-w-7xl mx-auto text-center py-20">
          ...
          <p className="text-ardoise mb-6">Retournez à la gestion des clients pour sélectionner des utilisateurs à comparer.</p>
          <NeonButton
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <Button
            variant="ghost"
            onClick={() => navigate(createPageUrl("AdminClients"))}
            className="text-ardoise hover:text-encre mb-4"
          >
            <ArrowLeft className="w-4 h-4 mr-2" />
            Retour
          </Button>
        ...
        {/* Résumé des profils */}
        <Card className="bg-gradient-to-br from-fond to-black border-menthe/30 mb-6">
          <CardHeader>
                  ...
                  <div key={u.id} className="p-4 bg-surface/50 rounded-lg border border-bord">
                    ...
                    {profil ? (
                      <div className="space-y-2">
                        <Badge className={`${profil.color} text-encre`}>
                          {profil.label}
                        </Badge>
                        <p className="text-ardoise text-sm">{profil.description}</p>
                      </div>
                    ) : (
                      <p className="text-ardoise text-sm">Profil non défini</p>
                    )}
                  </div>
                );
              })}
                ...
                <div key={idx} className="p-4 bg-surface/50 rounded-lg border border-red-500/30">
                  ...
                  <div className="space-y-2">
                    {discord.responses.map((r, rIdx) => (
                      <div key={rIdx} className="flex items-start gap-3 p-2 rounded bg-fond/50">
                        ...
                        <div>
                          <p className="text-craie text-sm font-medium">{r.user.full_name || r.user.email}</p>
                          <p className="text-ardoise text-sm">{r.label}</p>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        {/* Points d'accord */}
        {agreements.length > 0 && (
          <Card className="bg-gradient-to-br from-menthe/10 to-black border-menthe/30 mb-6">
            <CardHeader>
              <CardTitle className="text-encre flex items-center gap-2">
                <CheckCircle2 className="w-5 h-5 text-menthe" />
              ...
              {agreements.map((agreement, idx) => (
                <div key={idx} className="p-3 bg-surface/50 rounded-lg border border-menthe/30 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-menthe-clair text-sm">{agreement.label}</p>
                  </div>
                </div>
              ))}
            </CardContent>
        ...
        <Card className="bg-gradient-to-br from-fond to-black border-menthe/30">
          <CardHeader>
            <CardTitle className="text-encre">Comparaison détaillée des réponses</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {questions.map(q => {
                    ...
                    return (
                      <tr key={q.id} className={`border-b border-surface ${hasDiscord ? 'bg-red-900/10' : ''}`}>
                        <td className="p-3 text-craie">{q.question}</td>
                        {selectedUsers.map(u => {
                          const response = u.reponses_questionnaire?.[q.id];
                          return (
                            <td key={u.id} className="p-3">
                              {response ? (
                                <span className={`text-sm ${hasDiscord ? 'text-red-400' : 'text-ardoise'}`}>
