...
function Source({ s, onPreuve }) {
  ...
  return <button onClick={() => onPreuve(s)} className="font-mono text-[10.5px] tracking-[.06em] text-brume hover:text-encre whitespace-nowrap">{(s.document_nom || "").replace(/\.pdf$/i, "").slice(0, 26)}{s.page ? ` p. ${s.page}` : ""}</button>;
}
...
function GrilleCategorie({ g, onPreuve }) {
  return (
    <div className="mb-6">
      <p className="m-0 mb-2 text-[13px] font-semibold text-encre">{g.titre} <span className="text-brume font-normal">· {g.lignes.length} pièce{g.lignes.length > 1 ? "s" : ""}</span></p>
      <div className="overflow-x-auto border border-relief">
        <table className="border-collapse text-[12px] min-w-full">
          <thead><tr className="bg-fond">
            ...
            {g.colonnes.map((c) => <th key={c.id} title={c.question} className="text-left px-3 py-2 text-[11.5px] font-semibold text-encre border-b border-r border-relief min-w-[170px]">{c.libelle}</th>)}
          </tr></thead>
          <tbody>
            {g.lignes.map((l) => (
      ...
      <div className="px-6 py-8">
        ...
        <div className="mt-5 flex items-center gap-4">
          {enCours ? <span className="inline-flex items-center gap-2 text-[13px] text-ardoise"><Loader2 className="w-4 h-4 animate-spin" /> {e.remplissage.fait}/{e.remplissage.total ?? "…"} — {e.remplissage.document || "lecture"}</span>
            : e.progression.presents_etape ? <button onClick={() => lancer.mutate(2)} disabled={apercu || lancer.isPending} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-[10px] bg-encre text-fond text-[13px] font-semibold hover:bg-[#ffffff] disabled:opacity-40">Lire l'immeuble et la copropriété</button> : null}
        </div>
      </div>
    );
  }
    ...
    <>
      ...
      <div className="px-6 max-md:px-4 flex items-center justify-between gap-4 border-b border-trait">
        <div className="flex gap-6">
          {[["blocs", "Synthèse"], ["grilles", "Extraction"]].map(([id, l]) => (
            <button key={id} onClick={() => setVue(id)} className={`relative py-3 text-[14px] after:absolute after:left-0 after:right-0 after:-bottom-px after:h-[2px] after:bg-encre after:origin-left after:scale-x-0 after:transition-transform ${vue === id ? "text-encre font-semibold after:scale-x-100" : "text-brume hover:text-craie"}`}>{l}</button>
          ))}
        </div>
      ...
      {vue === "grilles" ? (
        <div className="px-6 max-md:px-4 py-5">
          ...
          {e.grilles.length ? e.grilles.map((g) => <GrilleCategorie key={g.id} g={g} onPreuve={onPreuve} />) : <p className="m-0 text-[13px] text-ardoise">Aucune pièce lue.</p>}
        </div>
      ) : (
        <>
          <Section id="copro" titre="1 · Copropriété">
            {questions.map((q) => (
              <div key={q.question} className="grid grid-cols-[minmax(220px,1fr)_2fr_130px] max-md:grid-cols-1 gap-x-6 gap-y-1 py-3 border-t border-relief first:border-t-0 items-baseline">
                <span className="text-[14px] text-craie">{q.question}</span>
                <p className="m-0 text-[14px] leading-[1.55] text-craie"><span className="font-semibold text-encre">{q.tete}</span> — {q.reponse} <Source s={q.source} onPreuve={onPreuve} /></p>
              ...
              <tbody>
                {bien.map((d) => (
                  <tr key={d.sujet} className="border-t border-relief">
                    <td className="py-3 pr-4 text-[14px] text-craie whitespace-nowrap">{d.sujet}</td>
                    <td className="py-3 pr-4 text-[14px] text-encre">{d.resultat} <Source s={d.source} onPreuve={onPreuve} /></td>
                    <td className={`py-3 pr-4 text-[14px] ${d.statut === "action" ? "text-encre" : "text-ardoise"}`}>{d.action}</td>
                ...
                <tbody>
                  {surfaces.sources.map((x, i) => (
                    <tr key={i} className="border-t border-relief">
                      <td className="py-3 pr-4 text-[14px] text-craie">{x.document_id ? <button onClick={() => onPreuve(x)} className="hover:text-encre text-left">{x.categorie === "Annonce" ? x.source : x.categorie}</button> : x.source}</td>
                      <td className="py-3 pr-4 text-[14px] text-encre">{x.perimetre}</td>
                      ...
                      <td className="py-3 text-[13px] text-ardoise">{x.observation}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ) : <p className="m-0 text-[13px] text-ardoise">Aucune surface lue.</p>}
            {!surfaces.incoherence && surfaces.sources.length > 0 && <p className="m-0 mt-3 text-[13px] text-ardoise">{surfaces.lecture}</p>}
          </Section>

          <Section id="marche" titre="4 · Marché">
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-x-8 gap-y-5">
              {marche.kpis.map((k) => (
                <div key={k.libelle}><Mono>{k.libelle}</Mono><p className={`m-0 mt-2 text-[20px] font-light tabular-nums ${k.valeur ? "text-encre" : "text-brume"}`}>{k.valeur || "—"}</p><p className="m-0 mt-1 text-[12px] text-brume">{k.detail}</p></div>
              ))}
            </div>
            {marche.commune && <p className="m-0 mt-5 text-[13px] text-ardoise">{marche.commune}{marche.revenu_median ? ` · revenu médian ${Math.round(marche.revenu_median).toLocaleString("fr-FR")} €` : ""}</p>}
            {marche.contexte ? <p className="m-0 mt-3 text-[14px] leading-[1.65] text-craie whitespace-pre-wrap max-w-[860px]">{marche.contexte.resume}</p> : null}
            <p className="m-0 mt-3 text-[14px] leading-[1.65] text-craie max-w-[860px]">{marche.question}</p>
          </Section>
          ...
          <Section id="synthese" titre="5 · Synthèse · factuel, le jugement est à l'étape 3" sansFilet>
            {synthese.map((x) => (
              <div key={x.sujet} className="grid grid-cols-[24px_150px_1fr] max-md:grid-cols-[24px_1fr] gap-x-4 py-3 border-t border-relief first:border-t-0 items-baseline">
                ...
                <span className="text-[14px] font-semibold text-encre">{x.sujet}</span>
                <span className="text-[14px] text-craie max-md:col-span-2">{x.texte}</span>
              </div>
            ))}
          </Section>
        </>
