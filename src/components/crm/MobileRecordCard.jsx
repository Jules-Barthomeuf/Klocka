...
export default function MobileRecordCard({ title, fields = [], onEdit, onDelete }) {
  return (
    <div className="rounded-md border border-[#22262d] bg-[#0f1114]/60 p-4">
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="text-[#f2f3f5] font-semibold text-sm break-words flex-1 min-w-0">{title || "-"}</div>
        {(onEdit || onDelete) && (
      ...
      <div className="space-y-2">
        {fields.map((f, i) => (
          <div key={i} className="flex items-start justify-between gap-3 text-sm">
            <span className="text-[#9298a6] text-xs uppercase tracking-wide flex-shrink-0 pt-1">{f.label}</span>
            <div className="text-[#f2f3f5] text-right break-words min-w-0">{f.value ?? "-"}</div>
          </div>
        ))}
      </div>
    </div>
