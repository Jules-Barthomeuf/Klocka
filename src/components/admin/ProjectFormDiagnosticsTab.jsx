    ...
    <div className="space-y-6 mt-6">
          ...
          <VerificationField fieldKey="dpe_note" formData={formData} setFormData={setFormData}>
            <FField label="Note DPE">
              <Select value={formData.dpe_note || ""} onValueChange={(value) => setFormData({...formData, dpe_note: value})}>
                <SelectTrigger className="bg-transparent border-none text-encre p-0 h-auto"><SelectValue placeholder="Sélectionner une note" /></SelectTrigger>
                <SelectContent>
            ...
            {["A","B","C","D","E","F","G"].map((note, idx) => {
              ...
              return (
                <div key={note} className={`flex items-center gap-2 transition-all ${isActive ? 'scale-105' : 'opacity-60'}`} style={{ width: widths[idx] }}>
                  <div className="flex-1 h-8 flex items-center justify-between px-3 rounded-r-lg text-encre text-sm" style={{ backgroundColor: colors[idx] }}>
                    <span>{note}</span>
      ...
      <div className="space-y-4 pt-6 border-t border-trait">
        <div className="flex items-center justify-between">
          <Label className="text-encre">Notes diagnostique (autres informations)</Label>
          <Button type="button" variant="outline" size="sm" onClick={() => setFormData({...formData, notes_diagnostique: [...(formData.notes_diagnostique || []), { titre: "", contenu: "" }]})} className="border-trait text-encre/30 hover:text-encre hover:border-bord-vif">
        ...
        {(!formData.notes_diagnostique || formData.notes_diagnostique.length === 0) && (
          <p className="text-ardoise text-sm text-center py-2">Aucune note. Cliquez sur le bouton ci-dessus pour en ajouter.</p>
        )}
      </div>
    </div>
  );
